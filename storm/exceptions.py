...
class StormError(with_metaclass(ABCMeta, Exception)):
    pass
    ...
    for exception in (Error, Warning, DatabaseError, InternalError,
        ...
        if (module_exception is not None and
            ...
            if six.PY2:
                ...
                # XXX virtual subclasses are still ignored for exception handling
                # (https://bugs.python.org/issue12029).
                try:
                    module_exception.__bases__ += (exception, )
                except TypeError:
                    tmp_exc = type(module_exception.__name__, (module_exception, ), {})
                    setattr(module, module_exception.__name__, tmp_exc)
