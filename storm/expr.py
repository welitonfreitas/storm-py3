...
class Compile(object):
    ...
    def __call__(self, expr, state=None, join=u", ", raw=False, token=False):
        ...
        if (expr_type is SQLRaw or
            raw and (expr_type is six.binary_type or expr_type is six.text_type)):
            return expr

        if token and (expr_type is six.binary_type or expr_type is six.text_type):
            expr = SQLToken(expr)
        ...
        if expr_type is tuple or expr_type is list:
            ...
            for subexpr in expr:
                ...
                if subexpr_type is SQLRaw or raw and (subexpr_type is six.binary_type or
                                                      subexpr_type is six.text_type):
                    statement = subexpr
                    ...
                    if token and (subexpr_type is six.text_type or
                                  subexpr_type is six.binary_type):
...
@compile_python.when(float, type(None), *(six.string_types + six.integer_types))
