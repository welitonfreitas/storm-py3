...
class StoreTest(object):
    ...
    def test_reload_not_changed(self):
        ...
        self.store.execute("UPDATE foo SET title='Title 40' WHERE id=20")
        self.store.reload(foo)
        for variable in list(get_obj_info(foo).variables.values()):
    ...
    def test_string_reference(self):
        class Base(with_metaclass(PropertyPublisherMeta, object)):
            pass
...
class EmptyResultSetTest(object):
    ...
    def test_values_no_columns(self):
        self.assertRaises(FeatureError, list, list(self.result.values()))
        self.assertRaises(FeatureError, list, list(self.empty.values()))
